...
class GeneticAlgorithm:
    ...
    def _sort(self):
        self._individuals.sort(key=lambda i: i.fit(), reverse=True)
    ...
    def start(self):
        while self._curr_generation < self._max_generations:
            while len(self._individuals) < self._max_individuals:
                child1, child2 = Individual.crossover(random.choice(self._individuals), random.choice(self._individuals))
